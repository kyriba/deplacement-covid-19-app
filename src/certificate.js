...
import { get, getAll } from './dom-utils'
...
function saveProfile () {
  for (const field of getAll('#form-profile input')) {
...
function getAndSaveReasons () {
  const values = getAll('input[name="field-reason"]:checked')
    .map(x => x.value)
    .join('-')
...
if (isFacebookBrowser()) {
  get('#alert-facebook').value = 'ATTENTION !! Vous utilisez actuellement le navigateur Facebook, ce générateur ne fonctionne pas correctement au sein de ce navigateur ! Merci d\'ouvrir Chrome sur Android ou bien Safari sur iOS.'
  get('#alert-facebook').classList.remove('d-none')
}

function addSlash () {
  get('#field-birthday').value = get('#field-birthday').value.replace(/^(\d{2})$/g, '$1/')
  get('#field-birthday').value = get('#field-birthday').value.replace(/^(\d{2})\/(\d{2})$/g, '$1/$2/')
}

get('#field-birthday').onkeyup = function () {
...
get('#generate-btn').addEventListener('click', async event => {
